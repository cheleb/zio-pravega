...
object ReaderSettingsBuilder {
  ...
  def apply(): ReaderSettingsBuilder = apply(ConfigFactory.load.getConfig(ReaderSettingsBuilder.configPath))
...
/**
 * Table Writer settings that must be provided to @see Sink [[akka.stream.alpakka.pravega.scaladsl.Pravega#tableSink]]
 * or @see Flow [[akka.stream.alpakka.pravega.scaladsl.Pravega#tableflow]]
 *
 * Built with @see [[WriterSettingsBuilder]]
 *
 */
...
private[pravega] object ConfigHelper {
  ...
  def extractString[Builder](path: String)(f: String => Builder)(implicit config: Config): Unit =
    if (config.hasPath(path))
      f(config.getString(path)): Unit
  def extractBoolean[Builder](path: String)(f: Boolean => Builder)(implicit config: Config): Unit =
    if (config.hasPath(path))
      f(config.getBoolean(path)): Unit
  def extractInt[Builder](path: String)(f: Int => Builder)(implicit config: Config): Unit =
    if (config.hasPath(path))
      f(config.getInt(path)): Unit
  def extractLong[Builder](path: String)(f: Long => Builder)(implicit config: Config): Unit =
    if (config.hasPath(path))
      f(config.getLong(path)): Unit
  def extractDuration[Builder](path: String)(f: Duration => Builder)(implicit config: Config): Unit =
    if (config.hasPath(path))
