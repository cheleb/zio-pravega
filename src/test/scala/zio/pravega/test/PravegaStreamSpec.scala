...
object PravegaStreamSpec extends PravegaIT { // */ DefaultRunnableSpec {
  ...
  val spec: Spec[Has[Clock] with Has[Console] with Has[System] with Has[Random], TestFailure[Any], TestSuccess] =
    suite("Prvavega")(
      zio.test.test("publish and consume") {

        assertM(
          program.provideCustomLayer(
            Pravega.live(scope, writterSettings.clientConfig) ++ zio.test.environment.testEnvironment
          )
        )(equalTo(20))
      }
    )
